...
void ExportDialog::setupUI() {
    ...
    // ========== ФОРМАТ И ФАЙЛ ==========
    QGroupBox *fileGroup = new QGroupBox("Настройки экспорта");
    ...
    // Прокручиваемая область для чекбоксов столбцов
    ...
    // Заполняем чекбоксы столбцов
    setupColumnCheckboxes();
    
    // ========== ОПЦИИ ==========
    QGroupBox *optionsGroup = new QGroupBox("Дополнительные опции");
...
void ExportDialog::setupColumnCheckboxes() {
    ...
    int columns = (colCount + itemsPerColumn - 1) / itemsPerColumn; // ceil деление
...
bool ExportDialog::exportToPDF(const QString &filename, bool isPreview) {
    ...
    // Масштабирование если не помещается
    ...
    for (int row = 0; row < model->rowCount(); row++) {
        ...
        // Для предпросмотра ограничиваем количество строк
...
void ExportDialog::onExport() {
    ...
    if (success) {
        ...
        // Открыть файл после экспорта если выбрано
