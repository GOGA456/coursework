...
UserEditDialog::UserEditDialog(int userId, QWidget *parent)
    : QDialog(parent), userId(userId), user(nullptr) {
    
    setWindowTitle(userId == -1 ? "Новый пользователь" : "Редактирование пользователя");
...
void UserEditDialog::loadUserData() {
    ...
    if (user) {
        ...
        qDebug() << "UserEditDialog::loadUserData() loaded user id:" << user->getId() << "username:" << user->getUsername();
    } else {
...
void UserEditDialog::onSave() {
    qDebug() << "UserEditDialog::onSave() userId:" << userId;
    ...
    if (userId == -1) {
        ...
        qDebug() << "Current user ID:" << user->getId() << "username:" << user->getUsername();
        ...
        qDebug() << "Saving user id:" << user->getId();
...
void UserManagementPanel::setupUI() {
    ...
    // QPushButton *btnExport = new QPushButton("Экспорт");
    ...
    // controlLayout->addWidget(btnExport); // УБИРАЕМ
    ...
    model->setHorizontalHeaderLabels(QStringList() << "ID" << "Логин" << "ФИО" << "Роль" << "Статус");
    ...
    // УБИРАЕМ экспорт пользователей
    // connect(btnExport, &QPushButton::clicked, this, &UserManagementPanel::onExportUsers);
...
void UserManagementPanel::loadUsers() {
    ...
    for (User *user : users) {
        ...
        row.append(new QStandardItem(QString::number(user->getId())));
...
void UserManagementPanel::onDeleteUser() {
    ...
    int row = selection.first().row();
    int userId = model->item(row, 0)->text().toInt();
    QString username = model->item(row, 1)->text();
