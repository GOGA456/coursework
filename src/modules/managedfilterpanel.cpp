...
QString ManagedFilterPanel::buildFilter() const {
    ...
    // Дата - для PostgreSQL используем правильный синтаксис
    QDate from = dateFromFilter->date();
    QDate to = dateToFilter->date();
    if (from.isValid() && to.isValid() && from <= to) {
        // Вариант 1: Если created_at - timestamp
        conditions << QString("created_at >= '%1 00:00:00' AND created_at <= '%2 23:59:59'")
                      .arg(from.toString("yyyy-MM-dd"))
                      .arg(to.toString("yyyy-MM-dd"));
        
        // ИЛИ вариант 2: Если хотите именно по дате без времени
        // conditions << QString("created_at::date BETWEEN '%1' AND '%2'")
        //               .arg(from.toString("yyyy-MM-dd"))
        //               .arg(to.toString("yyyy-MM-dd"));
    }
    
    // Если нет условий - возвращаем пустую строку
