...
void LoginDialog::onLogin() {
    ...
    // Используем ORM для проверки пользователя
    User* user = User::findByUsername(username);
    
    if (user) {
        qDebug() << "User found. ID:" << user->getId() 
        ...
        if (User::checkPassword(username, password)) {
            ...
            qDebug() << "Login successful! User:" << fullName << "Role:" << userRole;
            delete user;
            accept();
        } else {
        ...
        // Пользователь не найден - создаем admin если это admin
        if (username.toLower() == "admin") {
            ...
            if (newUser.save()) {
                ...
                userRole = "admin";
                qDebug() << "Admin user created! ID:" << userId;
